...
void SentryFlicker() {
    ...
    if ((int)self->saved_nextthink % 2 == 0) {
        LightningToSpyEmp(1);
        T_Damage(self, PROG_TO_EDICT(self->s.v.enemy), PROG_TO_EDICT(PROG_TO_EDICT(self->s.v.enemy)->s.v.owner), 3);
        TF_T_Damage(self, PROG_TO_EDICT(self->s.v.enemy), PROG_TO_EDICT(PROG_TO_EDICT(self->s.v.enemy)->s.v.owner), 10, TF_TD_IGNOREARMOUR, 0 );
    }
